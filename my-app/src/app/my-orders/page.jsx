...
import { ShoppingBag, CheckCircle, Clock, Truck, XCircle, Search, ArrowLeft } from "lucide-react";
...
export default function OrdersAdmin() {
    ...
    return (
        <div className="bg-white rounded-2xl p-6 shadow-sm border border-gray-150">
            <div className="max-w-[800px] mx-auto pt-8 px-4 flex justify-between items-center print:hidden mb-2 ml-8">
                <Link
                    href="/"
                        ...
                        {filteredOrders.length === 0 ? (
                            ...
                            filteredOrders.map((order) => (
                                <tr key={order._id} className="hover:bg-gray-50/50 transition-colors duration-200">
                                    ...
                                    <td className="py-4">
                                        <Link href={`/invoice?id=${order._id}`} className="bg-orange-50 text-orange-600 px-4 py-2 rounded-xl text-xs font-semibold hover:bg-orange-100 transition-colors">
                                            Print Invoice
                                        </Link>
                                    </td>
